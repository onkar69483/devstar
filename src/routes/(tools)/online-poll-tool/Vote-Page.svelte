<script>
  ...
  function handleVote() {
    if (hasVoted) {
      ...
      if (selectedOption) {
        ...
        infoMessage = 'Please select an option before voting.';
      }
    }
  }
  ...
  function showResults() {
    infoMessage = 'Sharing poll';
  }

  function share(platform) {
    infoMessage = `Sharing poll on ${platform}...`;
  }
</script>
...
<div class="outer">
  <div class="inner">
    ...
    <div class="info-message {infoMessage ? 'show' : ''}">
      {infoMessage}
    </div>
...
.already-voted.show {
  ...
  .modal-content {
    ...
    font-weight: bold;
    color: rgb(209, 213, 219);
    border: 3px solid rgb(56, 124, 228);
  }
  .modal-button {
    margin-top: 10px;
    padding: 10px 20px;
